...
async def update_lobby_post(bot: Bot, lobby_id: int):
    ...
    reg_topic = get_lobby_registration_topic(mode)
    ...
    if msg_id:
        try:
            # Для редактирования сообщения в Telegram достаточно chat_id + message_id.
            # message_thread_id относится к отправке нового сообщения и не должен
            # передаваться в editMessageText. Из-за него старый код попадал в except,
            # удалял пост и создавал новый.
    ...
    msg = await bot.send_message(GROUP_CHAT_ID, text, reply_markup=keyboard, message_thread_id=reg_topic)
    ...
    while True:
        ...
        try:
            ...
            for lobby_id in lobbies:
                ...
                conn.commit()
                # Не удаляем пост: update_lobby_post отредактирует существующее
                # сообщение и сохранит его message_id.
                await update_lobby_post(bot, lobby_id)
...
def init_lobbies():
    ...
    # При запуске ничего не удаляем.
    # Всегда поддерживаем 6 обычных лобби + 3 отдельных Ban/Pick.
    specs = [
        ("5x5", TOPIC_REG_LOBBY_5x5, 0, "5x5 #1"),
        ("5x5", TOPIC_REG_LOBBY_5x5, 0, "5x5 #2"),
        ("2x2", TOPIC_REG_LOBBY_2x2, 0, "2x2 #1"),
        ("2x2", TOPIC_REG_LOBBY_2x2, 0, "2x2 #2"),
        ("1x1", TOPIC_REG_LOBBY_1x1, 0, "1x1 #1"),
        ("1x1", TOPIC_REG_LOBBY_1x1, 0, "1x1 #2"),
    ...
    for mode, tid, ban_pick, label in specs:
        c.execute(
            "SELECT id FROM lobbies WHERE mode=? AND thread_id=? AND ban_pick=? LIMIT 1",
            (mode, tid, ban_pick)
        )
        if c.fetchone():
            continue
