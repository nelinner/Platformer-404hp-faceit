...
def init_lobbies():
    ...
    c.execute("DELETE FROM lobbies WHERE thread_id NOT IN (8930,8920,8917)"); conn.commit()
    # Создаём по два независимых лобби на каждый режим.
    # Ранее пары были продублированы, но SELECT ниже не позволял создать
    # второе лобби с тем же mode/thread_id.
    for mode, tid in [("5x5", TOPIC_REG_LOBBY_5x5),
                      ("2x2", TOPIC_REG_LOBBY_2x2),
                      ("1x1", TOPIC_REG_LOBBY_1x1)]:
        ...
        if not c.fetchone():
            ...
            c.execute("INSERT OR IGNORE INTO lobbies (mode, thread_id, map_name, ban_pick) VALUES (?,?,?,0)", (mode, tid, map_name))
    conn.commit()
