...
async def generate_lobby_image(bot: Bot, lobby_id: int) -> Optional[BytesIO]:
    try:
        ...
        bp_row = c.fetchone()
        bp_label = "BAN/PICK" if bp_row and bp_row[0] else "STANDARD"
        draw.text((52,130), f"{mode} • {bp_label}", font=get_font(46), fill="#e0e0e0")
...
async def update_lobby_post(bot: Bot, lobby_id: int):
    """Create/update the persistent registration card for a lobby.

    The card is a Telegram photo message. Existing photo messages are edited in
    place; old text messages are migrated once to a photo message.
    """
    c.execute("SELECT mode, thread_id, match_number, ban_pick, message_id, map_name FROM lobbies WHERE id=?", (lobby_id,))
    row = c.fetchone()
    if not row:
        logging.warning(f"Лобби #{lobby_id} не найдено")
        return
    mode, thread_id, match_num, ban_pick, msg_id, map_name = row
    reg_topic = get_lobby_registration_topic(mode)

    c.execute("SELECT user_id FROM lobby_registrations WHERE lobby_id=? ORDER BY joined_at", (lobby_id,))
    players = [r[0] for r in c.fetchall()]

    status = "🎯 BAN/PICK" if ban_pick else "🟢 ОБЫЧНОЕ"
    caption = (
        f"🎮 Лобби {mode} #{lobby_id} · {status}\n"
        f"Матч #{match_num} · Игроки: {len(players)}/{MAX_PLAYERS[mode]}\n"
        f"Карта: {map_name or 'не выбрана'}\n"
        f"{'После набора игроков запускается Ban/Pick.' if ban_pick else 'После набора игроков запускается обычная жеребьёвка.'}"
    )
    ...
    if not img_data:
        logging.error(f"Не удалось сгенерировать карточку lobby #{lobby_id}")
        return
    img_data.seek(0)
    photo = BufferedInputFile(img_data.read(), filename=f"lobby_{lobby_id}.png")
    ...
    if msg_id:
        try:
            ...
            logging.warning(f"Не удалось отредактировать карточку lobby #{lobby_id}, message_id={msg_id}: {e}")
            # Старое сообщение может быть текстовым или удалённым. Мигрируем его.
            try:
                await bot.delete_message(chat_id=GROUP_CHAT_ID, message_id=msg_id)
            except Exception:
                pass
            c.execute("UPDATE lobbies SET message_id=NULL WHERE id=?", (lobby_id,))
            conn.commit()

    img_data = await generate_lobby_image(bot, lobby_id)
    if not img_data:
        logging.error(f"Не удалось повторно сгенерировать карточку lobby #{lobby_id}")
        return
    img_data.seek(0)
    photo = BufferedInputFile(img_data.read(), filename=f"lobby_{lobby_id}.png")
...
async def ban_map_callback(query: CallbackQuery, bot: Bot):
    ...
    if len(remaining) == 1:
        ...
        c.execute("UPDATE lobbies SET map_name=? WHERE id=?", (final_map, lobby_id))
        conn.commit()
...
# ------------------------------------------------------------
def init_lobbies():
    """Ensure exactly the requested lobby slots exist.

    Normal registration: 2 slots for each mode (6 total).
    Ban/Pick registration: 1 slot for each mode (3 total).
    All nine are independent rows even when they share the same Telegram topic.
    """
    ensure_lobby_ban_pick_column()

    targets = [
        ("5x5", TOPIC_REG_LOBBY_5x5, 0, 2),
        ("2x2", TOPIC_REG_LOBBY_2x2, 0, 2),
        ("1x1", TOPIC_REG_LOBBY_1x1, 0, 2),
        ("5x5", TOPIC_REG_LOBBY_5x5, 1, 1),
        ("2x2", TOPIC_REG_LOBBY_2x2, 1, 1),
        ("1x1", TOPIC_REG_LOBBY_1x1, 1, 1),
    ]

    for mode, tid, bp, wanted in targets:
        c.execute("SELECT id FROM lobbies WHERE mode=? AND thread_id=? AND COALESCE(ban_pick,0)=? ORDER BY id", (mode, tid, bp))
        ...
        while len(ids) < wanted:
            ...
            c.execute(
                ...
                (mode, tid, map_name, bp),
            )
            ids.append(c.lastrowid)
    conn.commit()

async def restore_all_lobby_posts():
    # На каждом старте синхронизируем все 9 регистрационных карточек.
    # Это также один раз мигрирует старые текстовые посты в изображения.
    c.execute("SELECT id FROM lobbies ORDER BY id")
