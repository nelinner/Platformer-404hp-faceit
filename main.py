...
async def update_lobby_post(bot: Bot, lobby_id: int):
    ...
    if msg_id:
        try:
            if photo:
                # Пытаемся отредактировать медиа (фото) существующего сообщения
                ...
                # Если изображение не удалось сгенерировать, редактируем текст
...
async def start_draw(bot: Bot, lobby_id: int, mode: str):
    ...
    if img_data:
        try:
            ...
            logging.exception("Не удалось отправить изображение жеребьёвки для lobby=%s match=%s", lobby_id, match_num)
    else:
        logging.error("generate_draft_image вернула None: lobby=%s match=%s", lobby_id, match_num)
