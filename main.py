...
from aiogram.types import (
    ...
    BufferedInputFile, FSInputFile, InputMediaPhoto, ChatPermissions,
)
...
ban_pick_tasks: dict[int, asyncio.Task] = {}
...
async def generate_draft_image(bot, lobby_id, mode, ct_ids, t_ids, map_name, match_num, host_id):
    try:
        ...
        if map_name:
            ...
            # Для ban/pick – рисуем заглушку с надписью "BAN/PICK"
            draw.rounded_rectangle([center_x+40,map_y,center_x+center_w-40,map_y+340], radius=8, fill=(30,35,45))
            draw.text((center_x+center_w//2,map_y+170), "BAN/PICK", font=get_font(60), fill=FACEIT_ORANGE, anchor="mm")
...
def ensure_lobby_ban_pick_column():
    try:
        ...
        if "ban_pick" not in columns:
            c.execute("ALTER TABLE lobbies ADD COLUMN ban_pick INTEGER NOT NULL DEFAULT 0")
            conn.commit()
            logging.info("Добавлена колонка lobbies.ban_pick")
    except Exception as e:
        logging.error(f"Не удалось добавить lobbies.ban_pick: {e}")

def lobby_uses_ban_pick(lobby_id: int) -> bool:
    try:
        c.execute("SELECT ban_pick FROM lobbies WHERE id=?", (lobby_id,))
        row = c.fetchone()
        return bool(row and row[0])
    except Exception as e:
        logging.error(f"Ошибка чтения ban_pick lobby #{lobby_id}: {e}")
        return False
...
async def update_lobby_post(bot: Bot, lobby_id: int):
    ...
    row = c.fetchone()
    if not row:
        logging.warning(f"Лобби #{lobby_id} не найдено")
        return
    mode, thread_id, match_num = row
    use_ban_pick = lobby_uses_ban_pick(lobby_id)
    ...
    text = (
        f"{header}\n"
        f"Игроки: {len(players)}/{MAX_PLAYERS[mode]}\n"
        f"——————\n" +
        ("\n".join(lines) if lines else "Пока никого нет.") +
        "\n——————\n" +
        (f"Карта будет определена через ban/pick после набора игроков." if use_ban_pick
         else f"После набора игроков будет выполнена обычная жеребьёвка." )
    )
    ...
    # Для ban/pick лобби изображение не нужно
    if use_ban_pick:
        photo = None
        caption = text
    else:
        # Пытаемся сгенерировать изображение лобби
        img_data = await generate_lobby_image(bot, lobby_id)
        if img_data is not None:
            img_bytes = img_data.read()
            photo = BufferedInputFile(img_bytes, filename=f"lobby_{lobby_id}.png")
            caption = text
        else:
            photo = None
            caption = text
...
def get_lobby_registration_topic(mode):
    if mode == "1x1":
        return TOPIC_REG_LOBBY_1x1
    elif mode == "2x2":
        return TOPIC_REG_LOBBY_2x2
    elif mode == "5x5":
        return TOPIC_REG_LOBBY_5x5
    else:
...
async def start_draw(bot: Bot, lobby_id: int, mode: str):
    ...
    c.execute("SELECT match_number, map_name, ban_pick FROM lobbies WHERE id=?", (lobby_id,))
    row = c.fetchone()
    if not row:
        logging.error(f"Лобби #{lobby_id} не найдено при start_draw")
        return
    match_num, map_name, ban_pick = row
    ...
    # Если ban_pick, не передаём карту (она ещё не выбрана)
    map_for_draft = map_name if not ban_pick else None

    img_data = await generate_draft_image(bot, lobby_id, mode, ct, t, map_for_draft, match_num, host_id)
    if img_data:
        try:
            img_data.seek(0)
            photo = BufferedInputFile(img_data.read(), filename=f"draft_{lobby_id}_{match_num}.png")
            await bot.send_photo(GROUP_CHAT_ID, photo, message_thread_id=TOPIC_DRAW)
        except Exception as e:
            logging.exception(f"Не удалось отправить изображение жеребьёвки: {e}")
    ...
    try:
        ...
        logging.exception(f"Не удалось отправить текст жеребьёвки: {e}")

    if ban_pick:
        try:
            await start_ban_pick(bot, lobby_id, match_id, ct, t, host_id)
        except Exception as e:
            logging.exception(f"Не удалось запустить ban/pick: {e}")
    else:
        try:
            await bot.send_message(
                GROUP_CHAT_ID,
                f"🗺 Матч #{match_num}: карта {map_name or 'не выбрана'}. Ban/Pick для этого лобби выключен.",
                message_thread_id=TOPIC_DRAW
            )
        except Exception as e:
            logging.exception(f"Не удалось отправить сообщение о карте: {e}")

    # Обновляем пост лобби в любом случае
    await update_lobby_post(bot, lobby_id)

# ------------------------------------------------------------
# Ban/Pick функций
# ------------------------------------------------------------
async def start_ban_pick(bot, lobby_id, match_id, ct_ids, t_ids, host_id):
    captain_ct = ct_ids[0]
    captain_t = t_ids[0]
    map_pool = list(MAP_URLS.keys())
    session_id = create_ban_pick_session(lobby_id, match_id, captain_ct, captain_t, map_pool, TOPIC_BAN_PICK)

    text = format_ban_pick_text(lobby_id, host_id, ct_ids, t_ids, captain_ct, captain_t, map_pool)
    keyboard = build_ban_pick_keyboard(map_pool, session_id)
    msg = await bot.send_message(GROUP_CHAT_ID, text, reply_markup=keyboard, message_thread_id=TOPIC_BAN_PICK)
    update_ban_pick_session(session_id, message_id=msg.message_id)
    await schedule_ban_pick_timer(bot, session_id, msg.message_id, GROUP_CHAT_ID, TOPIC_BAN_PICK)

def format_ban_pick_text(lobby_id, host_id, ct_ids, t_ids, captain_ct, captain_t, remaining_maps):
    ct_names = ", ".join([get_nickname(pid) for pid in ct_ids])
    t_names = ", ".join([get_nickname(pid) for pid in t_ids])
    captain_ct_name = get_nickname(captain_ct)
    captain_t_name = get_nickname(captain_t)
    host_name = get_nickname(host_id)
    maps_str = " | ".join(remaining_maps)
    return (
        f"[⚒] Ban pick | maps\n"
        f"lobby: {lobby_id} | host: {host_name}\n"
        f"——————\n"
        f"Список команд:\n"
        f"[👥] Counter-terrorists: {ct_names}\n"
        f"[👥] Terrorists: {t_names}\n"
        f"——————\n"
        f"[👑] Capitan CT: {captain_ct_name}\n"
        f"[👑] Capitan T: {captain_t_name}\n"
        f"Карты: {maps_str}"
    )

def build_ban_pick_keyboard(remaining_maps, session_id):
    buttons = []
    for map_name in remaining_maps:
        buttons.append([InlineKeyboardButton(text=map_name, callback_data=f"banmap_{session_id}_{map_name}")])
    return InlineKeyboardMarkup(inline_keyboard=buttons)

async def schedule_ban_pick_timer(bot: Bot, session_id: int, message_id: int, chat_id: int, thread_id: int):
    # Отменяем предыдущую задачу, если она есть
    old_task = ban_pick_tasks.pop(session_id, None)
    if old_task:
        old_task.cancel()

    session = get_ban_pick_session(session_id)
    if not session or session["status"] != "active":
        return

    current_team = session["current_team"]
    captain_id = session[f"captain_{current_team.lower()}"]
    ...
    ban_pick_tasks[session_id] = task

async def ban_pick_timeout(bot: Bot, session_id: int, message_id: int, chat_id: int, thread_id: int, team: str, captain_id: int):
    try:
        await asyncio.sleep(15)  # 15 секунд на выбор
    except asyncio.CancelledError:
        return

    # Проверяем, не сделан ли уже ход
    session = get_ban_pick_session(session_id)
    if not session or session["status"] != "active" or session["current_team"] != team:
        return

    remaining = json.loads(session["remaining_maps"])
    if not remaining:
        return

    # Автоматический выбор случайной карты для бана
    banned_map = random.choice(remaining)
    remaining.remove(banned_map)
    banned = json.loads(session["banned_maps"])
    banned.append(banned_map)

    next_team = "T" if team == "CT" else "CT"

    # Обновляем сессию
    update_ban_pick_session(
        session_id,
        banned_maps=json.dumps(banned),
        remaining_maps=json.dumps(remaining),
    ...
    if len(remaining) == 1:
        ...
        final_text = format_ban_pick_text(
            lobby_id, session["host_id"], ct_ids, t_ids,
            session["captain_ct"], session["captain_t"],
            remaining
        ) + f"\n\n✅ Финальная карта: {final_map}"
        try:
            await bot.edit_message_text(
                chat_id=chat_id,
                message_id=message_id,
            ...
            logging.error(f"Не удалось обновить сообщение ban/pick: {e}")
        ban_pick_tasks.pop(session_id, None)
        lobby_topic = get_lobby_registration_topic(get_lobby_mode(lobby_id))
        await bot.send_message(chat_id=chat_id, text=f"Карта выбрана: {final_map}", message_thread_id=lobby_topic)
        return

    # Если осталось больше одной карты, обновляем сообщение и запускаем следующий таймер
    new_text = format_ban_pick_text(
        lobby_id, session["host_id"], ct_ids, t_ids,
        session["captain_ct"], session["captain_t"],
        remaining
    )
    ...
    await schedule_ban_pick_timer(bot, session_id, message_id, chat_id, thread_id)

@dp.callback_query(F.data.startswith("banmap_"))
async def ban_map_callback(query: CallbackQuery, bot: Bot):
    ...
    session = get_ban_pick_session(session_id)
    if not session or session["status"] != "active":
        await query.answer("Сессия ban/pick завершена.", show_alert=True)
        return

    # Отменяем таймер, так как игрок сделал ход
    task = ban_pick_tasks.pop(session_id, None)
    if task:
        task.cancel()

    current_team = session["current_team"]
    captain_id = session[f"captain_{current_team.lower()}"]
    if query.from_user.id != captain_id:
        await query.answer(f"Сейчас банит капитан команды {current_team}.", show_alert=True)
        return

    remaining = json.loads(session["remaining_maps"])
    if map_name not in remaining:
        await query.answer("Карта уже забанена.")
        return
    ...
    next_team = "T" if current_team == "CT" else "CT"
    update_data = {
        "banned_maps": json.dumps(banned),
        "remaining_maps": json.dumps(remaining),
        "current_team": next_team,
    ...
    if len(remaining) == 1:
        ...
        update_data["final_map"] = final_map
        update_ban_pick_session(session_id, **update_data)
        ...
        lobby_topic = get_lobby_registration_topic(get_lobby_mode(lobby_id))
        await bot.send_message(GROUP_CHAT_ID, f"Карта выбрана: {final_map}", message_thread_id=lobby_topic)
        return
    else:
        update_ban_pick_session(session_id, **update_data)
        new_text = format_ban_pick_text(
...
async def admin_menu(query: CallbackQuery):
    ...
    if not query.from_user.username or not is_admin(query.from_user.username):
        await query.answer("Нет доступа."); return
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
...
async def admin_manage_map_lobby_list(query: CallbackQuery):
    ...
    for lid, mode, map_name in lobbies:
        status = "🎯 Ban/Pick" if lobby_uses_ban_pick(lid) else "🟢 Обычное"
        buttons.append([InlineKeyboardButton(
            text=f"Лобби {lid} ({mode}) — {status} — карта: {map_name}",
            callback_data=f"admin_maplobby_{lid}"
        )])
...
async def admin_manage_map_choose_map(query: CallbackQuery):
    ...
    bp_text = "🎯 Выключить Ban/Pick" if lobby_uses_ban_pick(lobby_id) else "🎯 Включить Ban/Pick"
    buttons.append([InlineKeyboardButton(text=bp_text, callback_data=f"admin_toggle_banpick_{lobby_id}")])
...
@dp.callback_query(F.data.startswith("admin_toggle_banpick_"))
async def admin_toggle_banpick(query: CallbackQuery, bot: Bot):
    if not is_owner_of_menu(query):
        await query.answer("Это меню другого пользователя.", show_alert=True)
        return
    if not query.from_user.username or not is_admin(query.from_user.username):
        ...
        return
    lobby_id = int(query.data.split("_")[-1])
    current = lobby_uses_ban_pick(lobby_id)
    c.execute("UPDATE lobbies SET ban_pick=? WHERE id=?", (0 if current else 1, lobby_id))
    conn.commit()
    await update_lobby_post(bot, lobby_id)
    await query.answer("Ban/Pick включён." if not current else "Ban/Pick выключен.")
    await admin_manage_map_choose_map(query)
...
async def admin_ban_player_reason(message: Message, state: FSMContext, bot: Bot):
    ...
    try:
        ...
        logging.error(f"Не удалось отправить отчёт о бане: {e}")
...
async def results_score(message: Message, state: FSMContext, bot: Bot):
    ...
    for uid, team in players:
        ...
        # Калибровка
        decrement_calibration_matches(uid)
        if get_calibration_matches_left(uid) == 0:
            update_mmr(uid, get_elo(uid))
...
async def export_db(message: Message):
    ...
    try:
        ...
        await message.answer_document(
            FSInputFile(DB_PATH, filename=os.path.basename(DB_PATH))
        )
    except Exception as e:
...
# ------------------------------------------------------------
def migrate_lobbies_unique_constraint():
    """Удаляет старое ограничение UNIQUE(mode, thread_id) из существующей БД."""
    import re

    try:
        c.execute("PRAGMA index_list('lobbies')")
        indexes = c.fetchall()
        ...
        for row in indexes:
            ...
            c.execute(f'PRAGMA index_info("{name.replace(chr(34), chr(34)*2)}")')
            cols = [r[2] for r in c.fetchall()]

            if cols != ["mode", "thread_id"]:
                continue

            # Если UNIQUE был создан отдельным CREATE UNIQUE INDEX —
            # его можно просто удалить.
            if origin == "c":
                c.execute(f'DROP INDEX "{name.replace(chr(34), chr(34)*2)}"')
                conn.commit()
                logging.info("Удалён индекс UNIQUE(mode, thread_id): %s", name)
                return

            # origin == "u" означает table-level UNIQUE constraint.
            if origin != "u":
                continue

            c.execute("SELECT sql FROM sqlite_master WHERE type='table' AND name='lobbies'")
            ...
            # Удаляем только table-level UNIQUE(mode, thread_id).
            new_sql = re.sub(
                r',\s*(?:CONSTRAINT\s+\S+\s+)?UNIQUE\s*\(\s*mode\s*,\s*thread_id\s*\)',
                '',
                create_sql,
                count=1,
                flags=re.IGNORECASE
            ...
            if new_sql == create_sql:
                logging.error(
                    "Не удалось изменить схему lobbies: UNIQUE(mode, thread_id) найден, "
                    "но его формат не распознан."
                )
                return
            ...
            # Пересоздаём только таблицу lobbies, сохраняя все данные.
            # Остальные таблицы/данные не трогаем.
            c.execute("PRAGMA foreign_keys=OFF")
            try:
                c.execute("DROP TABLE IF EXISTS lobbies_new")

                new_sql = re.sub(
                    r'^\s*CREATE\s+TABLE\s+(?:IF\s+NOT\s+EXISTS\s+)?["`]?lobbies["`]?',
                ...
                c.execute(new_sql)
                c.execute(
                    f"INSERT INTO lobbies_new ({quoted_columns}) "
                    f"SELECT {quoted_columns} FROM lobbies"
                )
                c.execute("DROP TABLE lobbies")
                c.execute("ALTER TABLE lobbies_new RENAME TO lobbies")
                conn.commit()
            finally:
                c.execute("PRAGMA foreign_keys=ON")

            logging.info("Миграция lobbies завершена: UNIQUE(mode, thread_id) удалён.")
            return

    except Exception:
        conn.rollback()
        logging.exception("Ошибка миграции UNIQUE(mode, thread_id).")
        raise


def init_lobbies():
    ensure_lobby_ban_pick_column()
    migrate_lobbies_unique_constraint()

    # При запуске удаляем все лобби, чтобы пересоздать с правильными топиками
    c.execute("DELETE FROM lobbies")
    conn.commit()

    specs = [
        # Обычные лобби (ban_pick=0) — каждое в своём топике
        ("5x5", TOPIC_LOBBY_5x5_1, 0, "5x5 #1"),
        ("5x5", TOPIC_LOBBY_5x5_2, 0, "5x5 #2"),
        ("2x2", TOPIC_LOBBY_2x2_1, 0, "2x2 #1"),
        ("2x2", TOPIC_LOBBY_2x2_2, 0, "2x2 #2"),
        ("1x1", TOPIC_LOBBY_1x1_1, 0, "1x1 #1"),
        ("1x1", TOPIC_LOBBY_1x1_2, 0, "1x1 #2"),
        # Ban/Pick лобби (ban_pick=1) — в регистрационных топиках
        ("5x5", TOPIC_REG_LOBBY_5x5, 1, "5x5 | Ban/Pick"),
        ("2x2", TOPIC_REG_LOBBY_2x2, 1, "2x2 | Ban/Pick"),
        ("1x1", TOPIC_REG_LOBBY_1x1, 1, "1x1 | Ban/Pick"),
    ]

    for mode, tid, ban_pick, label in specs:
        map_name = random.choice(list(map_images_cache.keys())) if map_images_cache else None
        c.execute(
            "INSERT INTO lobbies (mode, thread_id, map_name, ban_pick) VALUES (?,?,?,?)",
            (mode, tid, map_name, ban_pick)
        )
        logging.info("Создано лобби %s: id=%s", label, c.lastrowid)
